...
?>><?php
	...
	if (!empty($args['image'])) {
		...
		?><img src="<?php echo esc_url($args['image']); ?>" alt="'.esc_attr__('image', 'greenville').'"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
		if (!empty($args['link'])) {
			?></a><?php
		} else {
			?></div><?php
	...
	?><div class="sc_price_info"><?php
		...
		if (!empty($args['description'])) {
			?><div class="sc_price_description"><?php echo wp_kses(trx_addons_parse_codes($args['description']), 'greenville_kses_content'); ?></div><?php
		}
		if (!empty($args['price'])) {
			...
			?><div class="sc_price_price"><?php
				...
				?><span class="sc_price_value"><?php echo wp_kses($parts[0], 'greenville_kses_content'); ?></span><?php
				if (count($parts) > 1 && $parts[1]!='') {
					?><span class="sc_price_decimals"><?php echo wp_kses($parts[1], 'greenville_kses_content'); ?></span><?php
				}
			?></div><?php
		}
