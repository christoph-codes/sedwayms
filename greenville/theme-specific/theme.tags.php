...
if ( !function_exists('greenville_need_page_title') ) {
	function greenville_need_page_title() {
		return !is_front_page() && apply_filters('greenville_filter_need_page_title', true);
	}
}
...
if ( !function_exists('greenville_show_layout') ) {
	function greenville_show_layout($str, $before='', $after='') {
		if (trim($str) != '') {
...
if ( !function_exists('greenville_get_logo_image') ) {
	function greenville_get_logo_image($type='') {
		$logo_image = '';
		if (greenville_get_retina_multiplier(2) > 1)
			$logo_image = greenville_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '').'_retina' );
		if (empty($logo_image)) 
			$logo_image = greenville_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '') );
		return $logo_image;
	}
}

// Return header video (if set)
if ( !function_exists('greenville_get_header_video') ) {
	function greenville_get_header_video() {
		$video = '';
		if (apply_filters('greenville_header_video_enable', !wp_is_mobile() && is_front_page())) {
			if (greenville_check_theme_option('header_video')) {
				$video = greenville_get_theme_option('header_video');
...
if ( !function_exists('greenville_show_post_meta') ) {
	function greenville_show_post_meta($args=array()) {
		...
		?><div class="post_meta"><?php
			...
			if ( !empty($args['categories'])) {
				$cats = get_post_type()=='post' ? get_the_category_list(', ') : apply_filters('greenville_filter_get_post_categories', '');
				if (!empty($cats)) {
					?>
					<span class="post_meta_item post_categories"><?php greenville_show_layout($cats); ?></span>
					<?php
				}
			}
			// Post date
			if ( !empty($args['date'])) {
				$dt = apply_filters('greenville_filter_get_post_date', greenville_get_date());
				if (!empty($dt)) {
					?>
			...
			// Post counters
			greenville_show_layout(greenville_get_post_counters($args['counters']));

			// Socials share
			if ( !empty($args['share']) ) {
				greenville_show_share_links(array(
						'type' => 'drop',
						'caption' => esc_html__('Share', 'greenville'),
			...
			// Edit page link
			if ( !empty($args['edit']) ) {
				edit_post_link( esc_html__( 'Edit', 'greenville' ), '<span class="post_meta_item post_edit icon-pencil">', '</span>' );
			}
		?></div><!-- .post_meta --><?php
...
if ( !function_exists('greenville_show_post_featured') ) {
	function greenville_show_post_featured($args=array()) {

		$args = array_merge(array(
			'hover' => greenville_get_theme_option('image_hover'),	// Hover effect
			...
			'seo' => greenville_is_on(greenville_get_theme_option('seo_snippets')),
			'singular' => is_singular()						// Current page is singular (true) or blog/shortcode (false)
			), $args);

		if ( post_password_required() ) return;

		$thumb_size = !empty($args['thumb_size']) ? $args['thumb_size'] : greenville_get_thumb_size(is_attachment() ? 'full' : (is_single() ? 'huge' : 'big'));
		$post_format = str_replace('post-format-', '', get_post_format());
		$no_image = !empty($args['show_no_image']) ? greenville_get_no_image() : '';
		if ($args['thumb_bg']) {
			if (has_post_thumbnail()) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id(get_the_ID()), $thumb_size );
				$image = $image[0];
			} else if ($post_format == 'image') {
				$image = greenville_get_post_image();
				if (!empty($image)) 
					$image = greenville_add_thumb_size($image, $thumb_size);
			}
			if (empty($image))
				$image = $no_image;
			if (!empty($image))
				$args['class'] .= ($args['class'] ? ' ' : '') . 'post_featured_bg' . ' ' . greenville_add_inline_css_class('background-image: url('.esc_url($image).');');
		}
			...
			if ( is_attachment() ) {
				...
				<div class="post_featured<?php if ($args['class']) echo ' '.esc_attr($args['class']); ?>"<?php if ($args['seo']) echo ' itemscope itemprop="image" itemtype="//schema.org/ImageObject"'; ?>>
					...
					if (has_post_thumbnail() && $args['seo']) {
						$greenville_attr = greenville_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
						?>
			...
			if ($has_thumb 
				...
				if ( $args['thumb_bg'] ) {
					...
					if (!in_array($post_format, array('audio', 'video'))) {
						greenville_hovers_add_icons($args['hover']);
					}
					...
					if (($output=greenville_get_slider_layout(array('thumb_size'=>$thumb_size, 'controls'=>'yes', 'pagination'=>'yes'))) != '')
						greenville_show_layout($output);
					...
					if (!empty($image)) {
						...
						greenville_hovers_add_icons($args['hover'], array('image' => $image));
					}
				...
				// Put video under the thumb
					if ($post_format == 'video') {
						$video = greenville_get_post_video($post_content, false);
					if (empty($video))
							$video = greenville_get_post_iframe($post_content, false);
						if (empty($video)) {
							...
							if ( false === strpos( $post_content, 'wp-playlist-script' ) ) {
								...
								if (!empty($videos) && is_array($videos)) {
									$video = greenville_array_get_first($videos, false);
								}
							}
						}
					if (!empty($video)) {
						if ( $has_thumb ) {
							$video = greenville_make_video_autoplay($video);
							?><div class="post_video_hover" data-video="<?php echo esc_attr($video); ?>"></div><?php 
						}
						?><div class="post_video video_frame"><?php 
							if ( !$has_thumb ) {
								greenville_show_layout($video);
							}
						?></div><?php
					}
				...
				// Put audio over the thumb
						...
						$audio = greenville_get_post_audio($post_content, false);
					if (empty($audio))
							$audio = greenville_get_post_iframe($post_content, false);
						if (empty($audio)) {
							// Only get video from the content if a playlist isn't present.
							$post_content = apply_filters( 'the_content', get_the_content() );
							if ( false === strpos( $post_content, 'wp-playlist-script' ) ) {
								...
								if (!empty($audios) && is_array($audios)) {
									$audio = greenville_array_get_first($audios, false);
							}
							}
						}
						if (!empty($audio)) {
						?><div class="post_audio<?php if (strpos($audio, 'soundcloud')!==false) echo ' with_iframe'; ?>"><?php 
							...
							$media_author = greenville_get_theme_option('media_author', '', false, get_the_ID());
							$media_title = greenville_get_theme_option('media_title', '', false, get_the_ID());
							if ( !empty($media_author) && !greenville_is_inherit($media_author) ) {
								?><div class="post_audio_author"><?php greenville_show_layout($media_author); ?></div><?php
							}
							if ( !empty($media_title) && !greenville_is_inherit($media_title) ) {
								?><h5 class="post_audio_title"><?php greenville_show_layout($media_title); ?></h5><?php
							}
							// Display audio
							greenville_show_layout($audio); 
						?></div><?php
					}
				}
				}
				...
				greenville_show_layout($post_info);
				?></div><?php
			} else {
				// Put optional info block over the thumb
				greenville_show_layout($post_info);
...
if ( !function_exists('greenville_get_no_image') ) {
	function greenville_get_no_image($no_image='') {
		static $img = '';
		if (empty($img)) {
			$img = greenville_get_theme_option( 'no_image' );
			if (empty($img)) $img = greenville_get_file_url('images/no-image.jpg');
		}
...
if ( !function_exists('greenville_add_bg_in_post_nav') ) {
	function greenville_add_bg_in_post_nav() {
		if ( ! is_single() ) return;
	
		$previous = ( is_attachment() ) ? get_post( get_post()->post_parent ) : get_adjacent_post( false, '', true );
		$next     = get_adjacent_post( false, '', false );
		$css      = '';
		$noimg    = greenville_get_no_image();
		
		if ( is_attachment() && $previous->post_type == 'attachment' ) return;
	
		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), greenville_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), greenville_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		wp_add_inline_style( 'greenville-main', $css );
	}
}

// Show related posts
if ( !function_exists('greenville_show_related_posts') ) {
	function greenville_show_related_posts($args=array(), $style=1, $title='') {
		$args = array_merge(array(
			...
			<section class="related_wrap">
				<h3 class="section title related_wrap_title"><?php
					if (!empty($title))
						...
						esc_html_e('You May Also Like', 'greenville');
				?></h3>
...
if ( !function_exists('greenville_show_portfolio_posts') ) {
	function greenville_show_portfolio_posts($args=array()) {
		$args = array_merge(array(
			'cat' => 0,
			'parent_cat' => 0,
			'taxonomy' => 'category',
		...
		$blog_style = explode('_', empty($args['blog_style']) ? greenville_get_theme_option('blog_style') : $args['blog_style']);
		$style = $blog_style[0];
		...
		if ( !$args['echo'] ) {
			...
			$q_args = greenville_query_add_posts_and_cats($q_args, '', $args['post_type'], $args['cat'], $args['taxonomy']);
			if ($args['page'] > 1) {
				$q_args['paged'] = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = greenville_get_theme_option('posts_per_page');
			if ((int) $ppp != 0)
...
// AJAX handler for the greenville_ajax_get_posts action
if ( !function_exists( 'greenville_ajax_get_posts_callback' ) ) {
	add_action('wp_ajax_greenville_ajax_get_posts',			'greenville_ajax_get_posts_callback');
	add_action('wp_ajax_nopriv_greenville_ajax_get_posts',	'greenville_ajax_get_posts_callback');
	function greenville_ajax_get_posts_callback() {
		...
		if ((int)$id > 0) {
			...
			greenville_storage_set('blog_mode', 'blog');
			greenville_storage_set('options_meta', get_post_meta($id, 'greenville_options', true));
		}

		$response = array(
			'error'=>'', 
			'data' => greenville_show_portfolio_posts(array(
							'cat' => (int) sanitize_text_field($_REQUEST['cat']),
							'parent_cat' => (int) sanitize_text_field($_REQUEST['parent_cat']),
							'page' => (int) sanitize_text_field($_REQUEST['page']),
							'post_type' => trim(greenville_get_value_gpc('post_type')),
							'taxonomy' => trim(greenville_get_value_gpc('taxonomy')),
							'blog_style' => trim(greenville_get_value_gpc('blog_style')),
							'echo' => false
							)
						)
		);
		...
		if (empty($response['data'])) {
			$response['error'] = esc_html__('Sorry, but nothing matched your search criteria.', 'greenville');
		}
...
// Show pagination
if ( !function_exists('greenville_show_pagination') ) {
	function greenville_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = greenville_get_theme_option('blog_pagination');
		if ($pagination == 'pages') {
			the_posts_pagination( array(
				'mid_size'  => 2,
				'prev_text' => esc_html__( '<', 'greenville' ),
				'next_text' => esc_html__( '>', 'greenville' ),
				'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'greenville' ) . ' </span>',
			) );
			...
			if ($page_number < $wp_query->max_num_pages) {
				...
				<div class="nav-links-more<?php if ($pagination == 'infinite') echo ' nav-links-infinite'; ?>">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e('Load more posts', 'greenville'); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__('Previous posts', 'greenville') : esc_html__('Newest posts', 'greenville') ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__('Next posts', 'greenville') : esc_html__('Older posts', 'greenville'), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
